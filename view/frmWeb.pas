...
uses
  ...
  Vcl.OleCtrls, SHDocVw, Vcl.Imaging.jpeg, Vcl.ComCtrls, Vcl.Imaging.pngimage;
...
begin
  ...
  Except
    Application.MessageBox('Erro ao Atualizar Página! Ação cancelada!',PChar(MSG_CAPTION),MB_ICONERROR);
  End;
end;
...
begin
  ...
  Except
    Application.MessageBox('Erro ao avançar para a próxima página em cache! Ação cancelada!',PChar(MSG_CAPTION),MB_ICONERROR);
  End;
end;
...
begin
  ...
  Except
    Application.MessageBox('Erro ao acessar a página inicial! Ação cancelada!',PChar(MSG_CAPTION),MB_ICONERROR);
  End;
end;
...
begin
  Try
    webbrowser1.Stop;
  Except
    Application.MessageBox('Erro ao parar o carregamento da página! Ação cancelada!',PChar(MSG_CAPTION),MB_ICONERROR);
  End;
end;
...
begin
  Try
    webbrowser1.goback;
  Except
    Application.MessageBox('Erro ao voltar para a página anterior em cache! Ação cancelada!',PChar(MSG_CAPTION),MB_ICONERROR);
  End;
end;
